...
class EfinanceFetcher(BaseFetcher):
    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        - ETF: ef.fund.get_base_info
        - 股票: ef.stock.get_base_info
        """
        import efinance as ef
        ...
        try:
            ...
            if _is_etf_code(stock_code):
                logger.info(f"[API调用] ef.fund.get_base_info(fund_codes={stock_code}) 获取基金名称...")
                info = ef.fund.get_base_info(stock_code)
                name_keys = ['基金简称', '基金名称', '名称', 'name']
            else:
                logger.info(f"[API调用] ef.stock.get_base_info(stock_codes={stock_code}) 获取股票名称...")
                info = ef.stock.get_base_info(stock_code)
                name_keys = ['股票简称', '股票名称', '名称', 'name']
            ...
            logger.debug(f"[股票名称] efinance 获取 {stock_code} 名称失败: {e}")
            return None
