...
class DataFetcherManager:
    ...
    def get_daily_data(
        ...
        """
        errors = []
        from .akshare_fetcher import _is_etf_code

        fetchers_to_try = list(self._fetchers)
        if _is_etf_code(stock_code):
            # ETF 场景优先使用 Akshare（可返回完整 OHLC/成交量）；
            # 若失败再回退到 Efinance 的基金净值接口。
            def _etf_rank(fetcher: BaseFetcher) -> int:
            ...
            fetchers_to_try = sorted(fetchers_to_try, key=lambda f: (_etf_rank(f), f.priority))
            fetcher_order = ", ".join(f.name for f in fetchers_to_try)
            logger.info(f"[ETF路由] {stock_code} 数据源尝试顺序: {fetcher_order}")
    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        # ETF 优先走基础信息接口，避免实时行情接口失败导致名称无法解析。
        if is_etf:
            for fetcher in self._fetchers:
        ...
        # 非 ETF 或 ETF 基础信息失败后，尝试从实时行情中获取
