...
# ETF 实时行情缓存（按接口区分）
_etf_realtime_cache: Dict[str, Dict[str, Any]] = {
    'em': {
        'data': None,
        'timestamp': 0,
        'ttl': 1200,  # 20分钟缓存有效期
...
# ETF 名称缓存（来自 fund_name_em 全量列表）
_etf_name_cache: Dict[str, Any] = {
    'data': None,       # Dict[code, name]
    'timestamp': 0,
    'ttl': 86400,       # 24 小时
}
...
def _to_cn_exchange_symbol(stock_code: str) -> str:
    """
    将 6 位 A 股/ETF 代码转为带交易所前缀的代码。
...
class AkshareFetcher(BaseFetcher):
    ...
    def _fetch_etf_data(self, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        数据来源（按优先级）：
        1. ak.fund_etf_hist_em()
        2. ak.fund_etf_hist_sina()（兜底）
        ...
        methods = [
            ("东方财富", self._fetch_etf_data_em),
            ("新浪", self._fetch_etf_data_sina),
        ]
        last_error: Optional[Exception] = None

        for source_name, fetch_method in methods:
            try:
                logger.info(f"[ETF历史] 尝试使用 {source_name} 获取 {stock_code}...")
                df = fetch_method(ak, stock_code, start_date, end_date)
                if df is not None and not df.empty:
                    logger.info(f"[ETF历史] {source_name} 获取成功: {len(df)} 行")
                    return df
            except Exception as e:
                last_error = e
                logger.warning(f"[ETF历史] {source_name} 获取失败: {e}")

        if last_error:
            error_msg = str(last_error).lower()
            if any(keyword in error_msg for keyword in ['banned', 'blocked', '频率', 'rate', '限制']):
                raise RateLimitError(f"Akshare ETF 历史接口可能被限流: {last_error}") from last_error

        raise DataFetchError(f"Akshare 获取 ETF 数据失败: {last_error}")

    def _fetch_etf_data_em(self, ak, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """使用东财接口获取 ETF 历史数据。"""
        self._set_random_user_agent()
        self._enforce_rate_limit()
    ...
    def _fetch_etf_data_sina(self, ak, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """使用新浪接口获取 ETF 历史数据（东财不可用时兜底）。"""
        self._set_random_user_agent()
        self._enforce_rate_limit()

        symbol = _to_cn_exchange_symbol(stock_code)
        ...
        if df is None or df.empty:
            logger.warning(f"[API返回] ak.fund_etf_hist_sina 返回空数据, 耗时 {api_elapsed:.2f}s")
            return pd.DataFrame()

        # 新浪接口字段标准化（与 _normalize_data 兼容）
        if 'date' in df.columns:
            df = df.rename(columns={'date': '日期'})
        if 'day' in df.columns:
            df = df.rename(columns={'day': '日期'})
        rename_map = {
            'open': '开盘',
            'high': '最高',
            'low': '最低',
        ...
        if '日期' in df.columns:
            ...
            df = df[(df['日期'] >= start_ts) & (df['日期'] <= end_ts)].copy()
        ...
        if '成交额' not in df.columns and '成交量' in df.columns and '收盘' in df.columns:
            df['成交额'] = pd.to_numeric(df['成交量'], errors='coerce') * pd.to_numeric(df['收盘'], errors='coerce')

        logger.info(f"[API返回] ak.fund_etf_hist_sina 成功: 返回 {len(df)} 行数据, 耗时 {api_elapsed:.2f}s")
        logger.info(f"[API返回] 列名: {list(df.columns)}")
    ...
    def get_realtime_quote(self, stock_code: str, source: str = "em") -> Optional[UnifiedRealtimeQuote]:
        ...
        if _is_us_code(stock_code):
            ...
            return self._get_etf_realtime_quote(stock_code, source=source)
        else:
    ...
    def _get_etf_realtime_quote(self, stock_code: str, source: str = "em") -> Optional[UnifiedRealtimeQuote]:
        """
        获取 ETF 基金实时行情数据
        
        数据来源（自动回退）：
        1. ak.fund_etf_spot_em()
        2. ak.fund_etf_spot_ths()
        包含：最新价、涨跌幅、成交量、成交额、换手率等
        
        Args:
            stock_code: ETF 代码
            source: 偏好来源（em/sina/tencent）
        ...
        import akshare as ak
        circuit_breaker = get_realtime_circuit_breaker()
        preferred = ['ths', 'em'] if source in ('sina', 'tencent', 'ths') else ['em', 'ths']
        # 去重并保序
        preferred = list(dict.fromkeys(preferred))

        def _find_row(df: pd.DataFrame, code: str) -> Optional[pd.Series]:
            code_cols = ['代码', '基金代码', '证券代码', 'symbol', 'code']
            for col in code_cols:
                ...
                matched = df[normalized == code]
                if not matched.empty:
                    return matched.iloc[0]
            # 兜底：若只有一行，且查询失败，返回首行（避免字段命名差异导致 miss）
            if len(df) == 1:
                return df.iloc[0]
            return None
        ...
        for api_source in preferred:
            source_key = f"akshare_etf_{api_source}"
            if not circuit_breaker.is_available(source_key):
                logger.warning(f"[熔断] 数据源 {source_key} 处于熔断状态，跳过")
                continue

            cache = _etf_realtime_cache[api_source]
            current_time = time.time()
            if cache['data'] is not None and current_time - cache['timestamp'] < cache['ttl']:
                df = cache['data']
                logger.debug(f"[缓存命中] 使用缓存的ETF实时行情数据 ({api_source})")
            else:
                last_error: Optional[Exception] = None
                df = None
                for attempt in range(1, 3):
                    try:
                        ...
                        if api_source == 'em':
                            ...
                            logger.info(f"[API调用] ak.fund_etf_spot_ths() 获取ETF实时行情... (attempt {attempt}/2)")
                            df = ak.fund_etf_spot_ths()

                        api_elapsed = _time.time() - api_start
                        logger.info(f"[API返回] ETF实时行情({api_source}) 成功: 返回 {len(df)} 条, 耗时 {api_elapsed:.2f}s")
                        circuit_breaker.record_success(source_key)
                        break
                    except Exception as e:
                        last_error = e
                        logger.warning(f"[API错误] ETF实时行情({api_source}) 失败 (attempt {attempt}/2): {e}")
                        time.sleep(min(2 ** attempt, 5))

                if df is None:
                    circuit_breaker.record_failure(source_key, str(last_error))
                    logger.error(f"[API错误] ETF实时行情({api_source}) 最终失败: {last_error}")
                    df = pd.DataFrame()
            ...
            if df is None or df.empty:
                logger.warning(f"[实时行情] ETF实时行情数据为空({api_source})，跳过 {stock_code}")
                continue

            row = _find_row(df, stock_code)
            if row is None:
                logger.warning(f"[API返回] ETF实时行情({api_source}) 未找到 {stock_code}")
                continue

            # THS 源在枚举中没有独立值，复用 AKSHARE_SINA 标记为轻量源
            quote_source = RealtimeSource.AKSHARE_EM if api_source == 'em' else RealtimeSource.AKSHARE_SINA
            quote = UnifiedRealtimeQuote(
                code=stock_code,
                name=str(_pick(row, ['名称', '基金简称', '基金名称', 'name']) or ''),
                source=quote_source,
                price=safe_float(_pick(row, ['最新价', '现价', '最新', 'close'])),
                change_pct=safe_float(_pick(row, ['涨跌幅', '涨幅', 'changepercent'])),
                change_amount=safe_float(_pick(row, ['涨跌额', '涨跌', 'change'])),
                volume=safe_int(_pick(row, ['成交量', 'volume'])),
                amount=safe_float(_pick(row, ['成交额', '成交金额', 'amount'])),
                volume_ratio=safe_float(_pick(row, ['量比', 'volume_ratio'])),
                turnover_rate=safe_float(_pick(row, ['换手率', 'turnover_rate'])),
                amplitude=safe_float(_pick(row, ['振幅', 'amplitude'])),
                open_price=safe_float(_pick(row, ['今开', '开盘', 'open'])),
                high=safe_float(_pick(row, ['最高', 'high'])),
                low=safe_float(_pick(row, ['最低', 'low'])),
                total_mv=safe_float(_pick(row, ['总市值', 'total_mv'])),
                circ_mv=safe_float(_pick(row, ['流通市值', 'circ_mv'])),
                high_52w=safe_float(_pick(row, ['52周最高', 'high_52w'])),
                low_52w=safe_float(_pick(row, ['52周最低', 'low_52w'])),
            )

            logger.info(
                f"[ETF实时行情-{api_source}] {stock_code} {quote.name}: 价格={quote.price}, "
                f"涨跌={quote.change_pct}%, 换手率={quote.turnover_rate}%"
            )
            return quote

        logger.warning(f"[实时行情] ETF {stock_code} 所有实时接口均失败")
        return None
    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        ETF 优先级：
        1. ETF 实时接口（THS -> EM）
        2. ak.fund_name_em 全量列表
        """
        import akshare as ak

        if _is_etf_code(stock_code):
            # 1) 优先从 ETF 实时接口拿名称
            quote = self._get_etf_realtime_quote(stock_code, source="ths")
            ...
            current_time = time.time()
            if (
                _etf_name_cache['data'] is None
                or current_time - _etf_name_cache['timestamp'] >= _etf_name_cache['ttl']
            ):
                try:
                    self._set_random_user_agent()
                    self._enforce_rate_limit()
                    logger.info("[API调用] ak.fund_name_em() 获取基金名称列表...")
                    df = ak.fund_name_em()
                    mapping: Dict[str, str] = {}
                    if df is not None and not df.empty:
                        code_col = next((c for c in ['基金代码', '代码', 'code'] if c in df.columns), None)
                        name_col = next((c for c in ['基金简称', '基金名称', '名称', 'name'] if c in df.columns), None)
                        if code_col and name_col:
                            for _, row in df[[code_col, name_col]].dropna().iterrows():
                                ...
                                if code and name:
                                    mapping[code] = name
                    _etf_name_cache['data'] = mapping
                    _etf_name_cache['timestamp'] = current_time
                except Exception as e:
                    logger.debug(f"[股票名称] fund_name_em 获取失败: {e}")

            mapping = _etf_name_cache.get('data') or {}
        ...
        # 非 ETF：尝试轻量源实时行情名称
        quote = self.get_realtime_quote(stock_code, source="tencent")
        if quote and quote.name:
            return quote.name
        return None
