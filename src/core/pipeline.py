...
class StockAnalysisPipeline:
    ...
    def analyze_stock(self, code: str, report_type: ReportType) -> Optional[AnalysisResult]:
        ...
        """
        try:
            asset_type = "etf" if self._is_etf_code(code) else "stock"
            ...
            if not stock_name:
                try:
                    resolved_name = self.fetcher_manager.get_stock_name(code)
                    if resolved_name:
                        stock_name = resolved_name
                        logger.info(f"[{code}] 通过名称接口识别: {stock_name}")
                except Exception as e:
                    logger.debug(f"[{code}] 名称识别失败: {e}")

            # 如果还是没有名称，使用代码占位
            if not stock_name:
                stock_name = f'ETF{code}' if asset_type == "etf" else f'股票{code}'
    ...
    def _enhance_context(
        ...
        # ETF 的基金净值接口通常不提供真实 OHLC/成交量，避免将 0 成交量当作真实行情传给 LLM
        if asset_type == "etf":
            today = dict(enhanced.get('today') or {})
            has_realtime = bool(enhanced.get('realtime'))
            close_price = today.get('close')
            is_nav_like = (
                close_price is not None
                and today.get('open') == close_price
                and today.get('high') == close_price
                and today.get('low') == close_price
                and today.get('volume') in (None, 0, 0.0)
                and today.get('amount') in (None, 0, 0.0)
            )
            if is_nav_like and not has_realtime:
                enhanced['etf_nav_mode'] = True
                enhanced['data_missing'] = True
                today['open'] = None
                today['high'] = None
                today['low'] = None
                today['volume'] = None
