...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _format_prompt(
        ...
        """
        code = context.get('code', 'Unknown')
        is_etf = str(context.get('asset_type', '')).lower() == 'etf'
        code_label = "基金代码" if is_etf else "股票代码"
        name_label = "基金名称" if is_etf else "股票名称"
        unknown_prefix = "ETF" if is_etf else "股票"
        
        # 优先使用上下文中的股票名称（从 realtime_quote 获取）
        stock_name = context.get('stock_name', name)
        if not stock_name or stock_name in (f'股票{code}', f'ETF{code}'):
            stock_name = STOCK_NAME_MAP.get(code, f'{unknown_prefix}{code}')
            
        today = context.get('today', {})
        if not isinstance(today, dict):
            today = {}
        display_open = today.get('open')
        display_high = today.get('high')
        display_low = today.get('low')
        display_volume = today.get('volume')
        display_amount = today.get('amount')
        if is_etf and context.get('etf_nav_mode'):
            display_open = None
            display_high = None
            display_low = None
            display_volume = None
...
| 开盘价 | {display_open if display_open is not None else 'N/A'} 元 |
| 最高价 | {display_high if display_high is not None else 'N/A'} 元 |
| 最低价 | {display_low if display_low is not None else 'N/A'} 元 |
| 涨跌幅 | {today.get('pct_chg', 'N/A')}% |
| 成交量 | {self._format_volume(display_volume)} |
| 成交额 | {self._format_amount(display_amount)} |
...
"""
        ...
        if 'trend_analysis' in context:
            ...
            volume_row = (
                ""
                if is_etf and context.get('etf_nav_mode')
                else f"| 量能状态 | {trend.get('volume_status', '未知')} | {trend.get('volume_trend', '')} |"
            )
            prompt += f"""
### 趋势分析预判（基于交易理念）
| 指标 | 数值 | 判定 |
...
"""
        ...
        # 添加昨日对比数据
        if 'yesterday' in context and not (is_etf and context.get('etf_nav_mode')):
...
1. 🌍 **宏观驱动**：利率、美元、通胀、避险情绪
2. 📌 **基金要素**：规模变化、申赎、费率、跟踪标的
3. ⚠️ **ETF风险**：折溢价、流动性、跟踪误差
...
当前 ETF 可能仅有基金净值序列，缺少真实成交量和分时成交结构。
请 **忽略上述表格中的 N/A 数据**，重点依据 **【📰 舆情情报】** 与净值趋势进行分析。
在回答量能/换手相关问题时，请直接说明“数据缺失，无法判断”，**严禁编造数据**。
"""
            else:
                prompt += """
⚠️ **数据缺失警告**
...
### ⚠️ 重要：基金名称确认
如果上方显示的基金名称为"ETF{code}"或不正确，请在分析开头**明确输出该基金的正确中文全称**。

### 重点关注（必须明确回答）：
1. ❓ 净值趋势是否向上（短中期）？
2. ❓ 主要驱动因素是宏观变量还是市场情绪？
3. ❓ 当前风险更偏向回撤风险还是机会成本风险？
4. ❓ 是否存在流动性或折溢价风险？
5. ❓ 空仓者与持仓者各自的操作建议是什么？
