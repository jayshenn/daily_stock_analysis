    ...
    def search_comprehensive_intel(
        ...
        if is_etf:
            search_dimensions = [
                {
                    'name': 'latest_news',
                    'query': f"{stock_name} {stock_code} ETF 基金 最新 公告 规模 资金流向",
                    'desc': '最新消息'
                },
                {
                    'name': 'market_analysis',
                    'query': f"{stock_name} {stock_code} 跟踪指数 成分 持仓 权重",
                    'desc': '标的分析'
                },
                {
                    'name': 'risk_check',
                    'query': f"{stock_name} {stock_code} ETF 折价 溢价 跟踪误差 流动性 风险",
                    'desc': '风险排查'
                },
                {
                    'name': 'macro_drivers',
                    'query': f"{stock_name} 对应标的 走势 宏观驱动 利率 美元 通胀 风险偏好",
                    'desc': '宏观驱动'
                },
                {
                    'name': 'fund_profile',
                    'query': f"{stock_name} 管理费 托管费 申赎 费率 规模 变动",
                    'desc': '基金要素'
                },
            ]
        else:
